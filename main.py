...
def show_all_docs_info():
    a= []
    for current_document in documents:
        a.append(show_document_info(current_document))
    return a
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            a, b = move_doc_to_shelf(user_doc_number, user_shelf_number)
            print('Документ номер "{}" был перемещен на полку номер "{}"'.format(a, b))
